...
public class OPPatientEligibility extends Support {
	WebDriver driver;
	WebDriverWait wait;
	...
	public OPPatientEligibility(WebDriver driver){
	...
	{
		List<String> CellValues = ExcelUtilities.getCellValuesInExcel("OPTESTDATA.xlsx", "PATIENTELEOPTIONS", columnNumber);
	...
	{
		String sName = ExcelUtilities.getKeyValueFromExcelWithPosition("OPTESTDATA.xlsx", "PATIENTSELE", "SupplementaryName", colNumber);
		String sTown = ExcelUtilities.getKeyValueFromExcelWithPosition("OPTESTDATA.xlsx", "PATIENTSELE", "SupplementaryTown", colNumber);
	...
	{
		String bName = ExcelUtilities.getKeyValueFromExcelWithPosition("OPTESTDATA.xlsx", "PATIENTSELE", "BeneficiaryName", colNumber);
		String bDOB = ExcelUtilities.getKeyValueFromExcelWithPosition("OPTESTDATA.xlsx", "PATIENTSELE", "BeneficiaryDOB", colNumber);
	...
	public OPPatientDeclaration PatientEligibilityDetailsEntered(int colNumber) throws InterruptedException, IOException
	{
	...
	public OPPatientDeclaration PatientEligibilityDetailsEntered(String colName,String file,String sheet) throws InterruptedException, IOException
	{
		OPHelpers.selectProvidedOptions(colName,file,sheet,driver);
		enterSupplimentaryDetails(colName);
		enterBeneficiaryDetails(colName);
		clickOnSaveandNextButton();
		Thread.sleep(2000);
	...
	public void enterSupplimentaryDetails(String colName)
	{
		String sName = ExcelUtilities.getKeyValueByPosition("OPTESTDATA_New.xlsx", "PATIENTSELE", "SupplementaryName", colName);
		String sTown = ExcelUtilities.getKeyValueByPosition("OPTESTDATA_New.xlsx", "PATIENTSELE", "SupplementaryTown", colName);
		scrolltoElement(driver, supplimentaryName);
		Support.enterDataInTextField(supplimentaryName, sName, wait);
		Support.enterDataInTextField(supplimentaryTown, sTown, wait);
	}
	
	public void enterBeneficiaryDetails(String colName)
	{
		String bName = ExcelUtilities.getKeyValueByPosition("OPTESTDATA_New.xlsx", "PATIENTSELE", "BeneficiaryName", colName);
		String bDOB = ExcelUtilities.getKeyValueByPosition("OPTESTDATA_New.xlsx", "PATIENTSELE", "BeneficiaryDOB", colName);
		scrolltoElement(driver, benName);
		String age = CommonFunctions.getDOB(bDOB);
		Support.enterDataInTextField(benName, bName, wait);
		Support.enterDataInTextField(benDOB, age, wait);
